...
def test_public_repo_inventory(file_contents):
    ...
    for filename in ["README.md", "README.md", "README_de.md", "llms.txt", "CHANGELOG.md"]:
...
def test_check_timestamp_parity(file_contents):
    """Verify that check timestamps are synchronized to 2026-08-21."""
    assert "2026-08-21" in file_contents["README.md"]
    assert "2026-08-21" in file_contents["README.md"]
    assert "21. August 2026" in file_contents["README_de.md"]
    assert "2026-08-21" in file_contents["llms.txt"]
    assert "2026-08-21" in file_contents["CHANGELOG.md"]
...
def test_ecosystem_cross_linking(file_contents):
    ...
    for org in ecosystem_orgs:
        assert org in file_contents["README.md"], f"Missing ecosystem org '{org}' in English profile"
        assert org in file_contents["README_de.md"], f"Missing ecosystem org '{org}' in German profile"
...
def test_banner_urls_and_assets(file_contents):
    ...
    assert "genotype-to-vcf/master/assets/banner.svg" in file_contents["README.md"]
    assert "VFDistiller/main/assets/banner.svg" in file_contents["README.md"]
    assert "genotype-to-vcf/master/assets/banner.svg" in file_contents["README_de.md"]
    assert "VFDistiller/main/assets/banner.svg" in file_contents["README_de.md"]


def test_research_use_boundary_disclaimer(file_contents):
    """Verify that Research Use Only boundary is stated."""
    assert "Research Use Only" in file_contents["README.md"]
    assert "Forschungszwecke" in file_contents["README_de.md"]
